...
def verify_real_data(source: Path) -> dict[str, object]:
    """Verify the exact real Kaggle file before model work."""
    if not source.is_file():
        raise FileNotFoundError(
            f"Real power data not found at {source}. "
            "Follow the starter README and download the named Kaggle dataset."
    ...
    if missing:
        raise ValueError(f"Missing expected columns: {sorted(missing)}")
    if rows != 2075259:
        raise ValueError(f"Expected 2075259 rows but found {rows}")
    dates = pd.read_csv(
        source, sep=";", usecols=["Date", "Time"], low_memory=False
    )
    timestamp = pd.to_datetime(
    ...
    return {
        "rows": rows,
        "columns": len(header),
        "time_range": [str(timestamp.min()), str(timestamp.max())],
    }
...
def make_lagged(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for lag in LAGS:
        ordered[f"lag_{lag}"] = ordered["global_active_power"].shift(lag)
    ordered["hour_of_day"] = ordered["timestamp"].dt.hour
    ordered["target_next"] = ordered["global_active_power"].shift(-1)
...
def build_candidate() -> object:
    ...
    return RandomForestRegressor(n_estimators=100, random_state=2026, n_jobs=-1)
...
def main() -> int:
    ...
    if args.check_data:
        checked = verify_real_data(args.source)
